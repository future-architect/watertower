...
func (o *GetIndexSearchURL) Build() (*url.URL, error) {
	...
	var _path = "/{index}/_search"

	index := o.Index
	if index != "" {
		_path = strings.Replace(_path, "{index}", index, -1)
	} else {
		return nil, errors.New("index is required on GetIndexSearchURL")
	}
...
func (o *GetIndexSearchURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetIndexSearchURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetIndexSearchURL")
	}

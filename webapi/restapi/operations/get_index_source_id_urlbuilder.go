...
func (o *GetIndexSourceIDURL) Build() (*url.URL, error) {
	...
	var _path = "/{index}/_source/{_id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{_id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetIndexSourceIDURL")
	}
	...
	if index != "" {
		...
		return nil, errors.New("index is required on GetIndexSourceIDURL")
	}
...
func (o *GetIndexSourceIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetIndexSourceIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetIndexSourceIDURL")
	}
